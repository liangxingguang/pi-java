 ...
 *
 * <p><strong>pi-java override</strong> (aligned with Codex CLI input
 * semantics): this same-package class shadows {@code tamboui-tui-0.4.0}'s
 * {@code EventParser} on the classpath so CR and LF stay distinguishable.
 * Vanilla TamboUI collapses both into a modifier-less {@link KeyCode#ENTER};
 * that makes plain Enter, Shift+Enter and Ctrl+J identical, which is why
 * "Shift+Enter newline" can never work through the app layer alone. Codex
 * CLI solves the same problem by normalizing terminal-emitted C0 characters
 * (LF = U+000A = Ctrl+J → {@code insert_newline}; see openai/codex #20555,
 * PR #20798). Upgrade diff point: only {@link #parseControlChar(int, Bindings)}
 * below differs from upstream 0.4.0.
 */
public final class EventParser {
    ...
    private static Event parseControlChar(int c, Bindings bindings) {
        switch (c) {
            ...
            case 9:
                // Tab (HT). Surface it as a plain CHAR('\t') instead of
                // KeyCode.TAB: TamboUI's focus router consumes KeyCode.TAB for
                // focus traversal before global handlers ever see it, which
                // would make slash-command completion via Tab impossible.
                return KeyEvent.ofChar('\t', bindings);
            case 10:
                // LF (U+000A): Windows ConPTY / Zed etc. report Shift+Enter as
                // LF, which is also Ctrl+J in C0 terms. Codex CLI normalizes
                // U+000A to logical Ctrl+J bound to insert_newline; we surface
                // it as a plain '\n' character so the app inserts a newline
                // instead of submitting. Enter (CR) below stays the submit key.
                return KeyEvent.ofChar('\n', bindings);
            case 13:
                return KeyEvent.ofKey(KeyCode.ENTER, bindings); // Enter (CR)
            case 27:
    ...
    private static Event parseCSIWithParams(String params, int terminator, Backend backend, Bindings bindings) throws IOException {
        ...
        // CSI-u (kitty keyboard protocol): ESC[<code>;<modifier>u. Windows
        // Terminal can be configured to send Shift+Enter as ESC[13;2u so the
        // modifier survives, which plain CR (\r) never carries.
        if (terminator == 'u') {
            return parseCsiU(params, bindings);
        }
    ...
    /** Parse CSI-u key sequences: ESC[<code>;<modifier>u (kitty keyboard protocol). */
    private static Event parseCsiU(String params, Bindings bindings) {
        String[] parts = params.split(";");
        int code;
        try {
            code = Integer.parseInt(parts[0]);
        ...
        KeyModifiers mods = parts.length > 1
            ? parseModifierCode(parts[1]) : KeyModifiers.NONE;

        return switch (code) {
            case 13 -> KeyEvent.ofKey(KeyCode.ENTER, mods, bindings);
            case 9 -> KeyEvent.ofKey(KeyCode.TAB, mods, bindings);
            case 27 -> KeyEvent.ofKey(KeyCode.ESCAPE, mods, bindings);
            case 127 -> KeyEvent.ofKey(KeyCode.BACKSPACE, mods, bindings);
            default -> parseCsiUPrintable(code, mods, bindings);
        };
    }

    /** Printable ASCII keys carried in CSI-u (letters, digits, punctuation). */
    private static Event parseCsiUPrintable(
            int code, KeyModifiers mods, Bindings bindings) {
        if (code >= 32 && code <= 126) {
            return KeyEvent.ofChar((char) code, mods, bindings);
        }
        return KeyEvent.ofKey(KeyCode.UNKNOWN, bindings);
    }
