...
> ## ⚠️ 参照物待定，方法论有效（2026-09-13）
>
> **本文的方法论与参照物无关，照用；本文的"参照物"部分已过期，需重定。**
>
> **参照物已分叉**：pi 现在有**三层** —— legacy `AgentEvent`（产品线在用）、harness `HarnessEvent`
> （22861 行实现，只有 `experimental/` 用）、**pico**（harness 的 clean-room 重写，仅 WP1）。
> 完整对照与待裁决项见 **`docs/27-pi-alignment-baseline.md`**。
>
> | 本文内容 | 状态 |
> |---|---|
> | §2 差分框架（归一化规则 / **P0-P1-P2 分诊** / 不可比清单） | ✅ **有效**，换任何一层照用 |
> | §4 `CrashInjectingPersister` 崩溃注入 | ✅ **有效** |
> | §5 四路载荷往返 | ✅ **有效** |
> | §7「**先建框架再改代码**」的执行顺序 | ✅ **有效** |
> | §0.1 子模块级判据表 | ❌ **作废** —— 基于 `936aff0` 旧检出（当时 harness 是空壳）。`harness-v2.md` 已删除 |
> | §0「②档无法差分」结论 | ❌ **反了** —— deferred / 清偿现在有真实代码可差分 |
> | §3 剧本 S1–S8 帧序 | ⚠️ **需重定** —— 原依 legacy `agent-loop.ts` 实读；若改跟 harness，要按 `HarnessEvent` 重写 |
> | §8 预判差异 #1（`agent_settled`） | ✅ 仍有效，且 harness 更彻底：连 `agent_start`/`agent_end` 都没有 |
>
> **恢复实施的前置**：先在 `docs/27` §6 裁决跟哪一层。
...
| `usage` 数值 | ~~降为 `{"usage":true}`~~ **本行已过期（两度）**：实现从未「降为 true」而是整键丢弃（`docs/29 §6`）；3a（`docs/31 §8.19`）起**全量渲染**（两侧 mock 恒零⇒差分空转）；**包H1 步 7 起逐字比对**，S15 专职守护（`docs/29 §10`） | 剧本桩两侧都确定性产出，无「模型/provider 不同」问题 |
