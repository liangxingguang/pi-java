...
| B45 | **`pi-java-tui` 的 `spotbugs:check` 有 4 条 finding，卡住全树 `clean verify`** —— `ChatScreen` 的 `runToolCalls` 是 `AT_NONATOMIC_OPERATIONS_ON_SHARED_VARIABLE`（`ChatScreen.java:124` 的 `runToolCalls++` ＋ `:151` 的 `resetRunTracking()` 清零），`assistantStreamed`（`:92`/`:109`）与 `thinkingRendered`（`:93`/`:122`）是 `AT_STALE_THREAD_WRITE_OF_PRIMITIVE` | `docs/33 §10.7`（实测输出） | 写点在 `onStreamEvent`（**包⑤ 之前**的老路径），字段出自 `d1e8d3a`／`b378b20`（2026-08-16 及更早）⇒ **非包⑥ 引入**（`git status --porcelain -- pi-java-tui/` = 0）。⚠️ **可能不是误报**：事件线程写 ＋ 提交线程清零 ＋ 渲染线程读，正是 memory「并发审计的单位是『共享对象 × 全部线程』」那一类。⚠️ **我不知道此前为何没被拦住**（`spotbugs:check` 确实绑在默认 `verify`，根 `pom.xml:172-180`）—— 没去追时间线。**未修**：TUI 是你明示要降级的模块，且修法涉及行为语义 ⇒ **待裁决**（修 / 挂白名单 / 只登记） |
