...
| **B** | **功能缺口**（pi 有、pi-java 无） | 30 | 我（另立包，多数需先出设计文档） |
...
| B3 | TUI/RPC 对 **auto_retry / summarization_retry 的渲染**（倒计时、`isRetrying`、isIdle 含重试） | `docs/31:1552` | 并入「命令/界面面」清点。⚠️ **2026-09-19 取证更正**：这行的措辞是**登记时的猜测** —— pi 的 `isRetrying` **没有任何消费者**（`agent-session.ts:2977` 只声明，src 全仓零读点），`isIdle` 只在扩展上下文里用；pi 的 `RpcSessionState` 里也**没有**这两个字段。RPC 的真正缺口是 `get_state` 的**四个状态字段在 pi-java 被写死**（`buildState():336-354` 给 `false`/`null`/`null`/`0`）＋ `auto_retry_end` 多一个键（= **B12**）。**包④ 设计见 `docs/31 §8.37`（待审）** |
