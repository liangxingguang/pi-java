...
> **A4 / B53 / B55 重新取证**（结论均有变，见各行）。重测另见 `docs/map/ANCHOR.md`。
...
> **2026-09-22 包 A0（`pi-java-ai` 硬故障与凭证小件）**：**B16 结案**（`sanitizeSurrogates`：
> 24 个落线点 ＋ oracle 差分证明；⚠️ 三处更正见 B16 行）；⇒ B 类 79→**83** 行（新登记
> B80–B83：Mistral 数组形态 content ／ pi OAuth 的两面 ／ OAuth 登录结果不在凭证链上 ／
> pi 自身四条缝的照缝裁决）、未结 52→**55**。设计/记录：`docs/43`（§8 提交清单 ·
> §9 实测校正 · §10 逐步台账）。⚠️ 本包还**改写了 `docs/41 §7.2` 的补全次序**（用户改判：
> ai 模块整体先行 ⇒ H2–H6 与后续梯队顺延）。

> **2026-09-22 包 H2（`pi-java-ai` 图片内容）**：**B17 结案**（共享闸 ＋ 四车道九个落线点）；
> **B14 收窄**（图片降级切片已落，只剩三条变换）；⇒ B 类 83→**86** 行（新登记 B84–B86：
> Google 工具结果**整条路径**缺失 ／ `read.ts` 侧两条 ／ H2 顺带发现的形状偏差与两处不可观察的能力门）、
> 未结 55→**58**。设计/记录：`docs/44`（§9 裁决与执行 · §10 实测校正 · §11 逐步台账）。
> ⚠️ 本包**拆掉了两条存量超限车道**（`AnthropicMessagesApi` 696→494 ／
> `OpenAICompletionsApi` 729→366，各抽出一个转换器类）。

> **2026-09-23 包B84（`pi-java-ai` Google 车道工具结果路径）**：**B84 结案**（`docs/45`，六提交）。
> 它同时**超出** B84 原记范围做了四件事（升 `google-genai` 1.15.0⇒1.72.0 ／ `functionCall.id` 门 ／
> 空白助手文本块跳过 ／ 拆 `GoogleMessageConverter` ＋ `ToolResultContent` 分支置空）——
> 理由逐条在 `docs/45 §3 D3–D7` 与 §9。
> **B86 追加第 3 例**（Google 车道侧能力门同形不可达）＋ **两条新登记**（`google-genai` native 未验证 ／
> 空白判据的 NBSP 缝）；**B14③ 行点名**（归一 toolCall id 不只关乎 anthropic）。
> **B 类行数不变（86）** —— 本包**不新开条目**，只结案 B84 并把发现并进既有条目。
> 设计/记录：`docs/45`（§9 裁决与执行 · §10 实测校正 · §11 逐步台账）。
...
| A4 | 运行中手动 `/compact` 的**中止语义** —— 原问法是「有没有门」 | `docs/31:2503`；**2026-09-20 已取证（`docs/40 §1.3`）** | ✅ **已取证，结论：pi 是 abort-first，不是 gate。** `compact()` 现于 `agent-session.ts:2089`，**第一句就是 `await this.abort()`**（`:2090`；⚠️ 该句**旧锚点 `:1968` 就有**，不是新加的），随后自建 `_compactionAbortController`。**本轮（`de2de549b`「close compaction cancellation races」，改该文件 156 行）新增 4 条取消语义**：① abort 信号打进鉴权取件 `_getSummarizationRequestAuth(model, signal)`（`:2099`，旧版不带 signal ⇒ 取消后仍可能挂在鉴权上，**这正是那条 race**）；② `session_before_compact` 事件新增 `signal: AbortSignal`（`:2130`）；③ `aborted` 判定**从字符串匹配改为信号权威**（`:2223`：旧 `message === "Compaction cancelled" \|\| error.name === "AbortError"` → 新 `signal.aborted \|\| cancelledByExtension`）；④ `_clearManualCompactionState()` 提到 `compaction_end` **之前**（`:2214`）。**pi-java 四者全无**：`MiscCommands.java:124` → `AgentSession.compact:641` → `harness.compact(laneName, settings)`，**既无 abort-first、也无 `abortCompaction`、更无任何取消语义**。修法是行为改动 ⇒ 见 F4（已裁「对齐 pi」） |
...
| B14 | **`transform-messages.ts` 的其余四条变换全部缺失**（一条聚合行） | §8.34.3（`docs/31:4242`） | ① 图片降级为占位文本（`transform-messages.ts:35-57`，按 `model.input` 判定）；② 跨模型剥离 toolCall 的 `thoughtSignature`（`:131-134`）；③ 跨模型归一 toolCall id（`:136-142`）；④ **孤儿 toolCall 合成 `toolResult`**（`:158-220`「No result provided」）＋ 跳过 `error`/`aborted` 助手消息（`:194-197`）。④ 是**真功能**、其余三条是清理 ⇒ 不塞进包②（会让 200 行变 800 行），另立。<br>✅ **①已随包 H2 落地**（`docs/44` 步1，`7d6e5ab`）：`TransformMessages.downgradeUnsupportedImages` ＋ `replaceImagesWithPlaceholder` 逐行照抄（含 `:29` 的第二处去重），`ModelInfo.supportsImageInput()` 承载能力位。⚠️ **本行收窄为②③④三条** —— 其中 **④ 是唯一的真功能**（孤儿 toolCall ⇒ provider 400），排期时优先 |
| B15 | **扩展开启（extended thinking）在生产上不可达** —— 目录的 `reasoning` 标记永远传不到请求 | §8.34.4-决策 5 | 链路逐段实测：`models.json` 的 `reasoning:true` 只落成 `ModelCapability.THINKING`（`ModelsJsonConfig:191-193`），`thinkingLevelMap` 硬写 `empty()`（`:208`）；`HarnessConfig.thinkingLevelMap` 默认 `empty()`（`:159`）且 `Builder.thinkingLevelMap` **零主源调用者**；`forLevel` 在空 map 上**恒返回 `ThinkingConfig.OFF`**（`ThinkingLevelMap:26-34`）⇒ `DefaultProviders:111-113` 门恒假 ⇒ `extra` 永无 `thinking.budgetTokens` ⇒ `AnthropicMessagesApi:269-276` 永不发 `thinking`。**`ThinkingLevelMap.of(` 亦只有测试调用者** ⇒ 非空 map 在生产上**不可构造**。根因＝「**目录元数据 → 请求路径**」这条通道**整体缺失**（B8 的 `compat` 是同一断链的**第二个**受害者）⇒ 两者应合为**一次**投送修复。**它改变「发什么请求」**（比包② 重）⇒ 须**自己一包**。⚠️ `StreamSimple:44` 虽手持 `ModelInfo`，但**生产上是死的**（3 个调用点全在 `StreamSimpleTest`）—— 别把它当接缝。**⚠️ 投送链那半边已由包② 修好（§8.34.11）**：`StreamRequest` 现带整个 `ModelInfo`、`DefaultProviders.streamBlocking` 已投真目录元数据 ⇒ **本行只剩「`thinkingLevelMap` 生产上不可构造」这半边**，难度显著下降。**✅ 已闭环**（包H5，2026-09-23，八提交 `2d229c9`..`b75450f`，设计/记录 `docs/46`）—— ⚠️ 实施中实测：`ThinkingLevelMap` 的形状本身也是错的（java 的 `Map<ThinkingLevel,ThinkingConfig>` 是 Phase 2a 的**发明**，pi 自 `80f06d363` 起一直是 `Partial<Record<ModelThinkingLevel, string|null>>`）⇒ 先重塑形状（删掉那个发明类型 `ThinkingConfig`），再做三分支。残留 9 条见 `docs/46 §7`（含**新登记**：`ModelThinkingLevels` 零生产调用者）。 |
| B16 | **`sanitizeSurrogates` 全仓无对应物** —— 出站文本未做孤对代理清理 | §8.34.11 | pi 在自己的**每个** API 适配器里都用它包裹出站 text/thinking（全仓 **57 处**调用；⚠️ **2026-09-20 更正：原记 54，实测 57**），pi-java 自己的模块**零处**（`.agents/` 里 vendored tamboui 的那几处是宽度计算，无关）。孤对代理字符会让请求体 JSON 非法 ⇒ provider 400。**跨车道**（不是 Anthropic 独有）⇒ 须先设计「在哪一层做一次」而不是逐适配器抄。<br>✅ **已实施（包 A0 步1–5，`docs/43`，2026-09-22）**：⚠️ **三处更正** —— ① 调用点逐行实读是 **46 处 / 9 文件**（不是 57）；② 裁决**不是**「在哪一层做一次」而是**逐车道字段级照抄**（四条车道里三条把序列化交给厂商 SDK，java 侧**不存在**可收口的单点；中央收口反而会**多**净化 pi 不净化的东西 ⇒ `docs/43 D1`）；③ java 落 **24 个落线点**（Anthropic 6 ／ completions 4 ／ responses 5 ／ Google 3 ／ openrouter-images 1 ／ Mistral 请求 4 ＋ 响应 1），差额 ＝ grammar 2 处（随 grammar 包）＋ 4 处「面不存在」＋ 3 处照缝（`docs/43 §6/§9`）。实现＝手写 code-unit 循环，正确性由 pi 正则作 oracle 双向差分证明（20 边界用例 ＋ 5000 随机串） |
| B83 | **pi 自身也有四条「不净化」的缝**（java 已照缝，故非 java 缺口） | 包 A0 实读（`docs/43 §6-8`、§9-1） | pi 的 `sanitizeSurrogates` **不是**全覆盖：① 普通工具参数的**重放**（`completions:1366`／`responses:322` `JSON.stringify`，即早已登记的 P5）＋ ② 签名驱动 reasoning 字段（`completions:1339`）＋ ③ google 的 `functionCall.args`（`:274`）。三条同根：**只有 grammar/custom 路径的工具入参才净化**（`completions:1359`／`responses:312`）。**裁决（`docs/43 D1`）：照缝移植** —— 多净化是行为偏离；三条各配 guard 夹具防「顺手补掉」。⇒ 若 pi 将来补上这三处，本行即触发同步 |
| B17 | **图片块在四条车道里被三条静默丢弃**（原记「Anthropic 车道独缺」） | §8.34.11；**2026-09-20 扩范围（`docs/40` 取证）** | `AnthropicMessagesApi.toBlockParams` 只处理 Text/Thinking/ToolUse ⇒ `ImageContent`/`UrlImageContent`/`DiffContent` **无声消失**。⚠️ **2026-09-20 更正两处**：① **不是 Anthropic 独缺** —— `OpenAICompletionsApi.buildParams`（`:417`/`:427`/`:330`）用 `extractText()` 把 user 与 toolResult **拍平成纯文本**、`MistralConversationsApi`（`:290`/`:330`）同样 ⇒ **四条车道丢三条**，pi 侧三条都映射（`openai-completions.ts:1248-1250`/`:1415-1416`、`mistral-conversations.ts:798`）；② **漏了 toolResult 路径** —— `toToolResultBlock`→`toTextBlocks`（`:536-545`）只收 `TextContent` ⇒ **`ReadTool`（`ReadTool.java:86` 返回 `ImageContent`）读一张图，模型完全看不到图**，而 pi 的 `convertContentBlocks`（`anthropic-messages.ts:121-160`，toolResult 调用点 `:1206`）两条路径都处理。**后果是静默数据丢失**（不报错、内容为空），是本台账里唯一的这一类。<br>✅ **已实施（包 H2，`docs/44`，2026-09-22，七提交 `af5139e..9bbcd48`）**：⚠️ **四处更正** —— ① 落地范围是**共享闸 ＋ 四车道九个落线点**（Anthropic 2 ／ completions 2 ／ Mistral 2 ／ responses 1 ＋ 闸 1），不是「三条车道」；② **Google 被实测证伪**：它的工具结果**整条路径**不存在（不是缺图片）⇒ 拆出去另立（**B84**）；③ 非视觉模型**必须**同包落 pi 的降级闸（`transform-messages.ts:35-57`），否则等于「用一个硬故障换另一个硬故障」⇒ 归 B14 的图片切片；④ 链路取证结论：缺口**只在出站那一跳** —— `ReadTool`→`ToolResult`→`PiToolRunner`→JSONL 双向四段全通 |
...
| B53 | ⚠️ **`find()` 与 `latest()` 同病**：`PersistentSessionRepositories.find()`（`:52`）**也走 `all()`** ⇒ `--session <id>` / `--session-id`（以及 `--fork/-r`）同样是 **O(全部会话文件)**，且同样**跨项目**。pi 的对应物 `findLocalSessionByExactId(id, cwd, sessionDir)` 是**项目内** | `docs/39 §6.4`（包⑫ 顺带发现，**未修**） | 与 A12 **同根但不同命**：`latest()` 的越界是纯 bug（pi 无此形状），而 `--session <id>` 的**跨项目查找**可能有人真在用 ⇒ 改它会改**行为裁决**，不能搭 A12 的车。**另立一包**：先取证 pi 的 `-r/--session` 有没有跨项目路径（`main.ts:432` 用的是 `findLocalSessionByExactId`），再定。<br>⚠️ **2026-09-20 取证完成，本行定性翻转** —— **台账原记的「pi 是项目内」被证伪**：pi 的 `resolveSessionPath`（`main.ts:251-281`）是**三级阶梯**，**第 ③ 级就是跨项目 `listAll`**（`:272`；⚠️ 该级**旧锚点就已存在**，不是本轮加的）。本轮新增的是**第 ① 级**：`SessionManager.findById`（`session-manager.ts:1732`）**只读 header**。⇒ **真差距是「pi-java 直接跳到第 ③ 级、缺 ①②」＝ 次序与成本，不是「跨项目＝bug」**。`findLocalSessionByExactId`（`main.ts:242`）行号未变，但已从 `async` 改同步、内部走 `findById`（header-only） |
...
| B55 | **pi 的会话列表 API 带 `onProgress`**（`session-manager.ts:812`、**`:768`** —— ⚠️ **2026-09-20 更正：原记 `:772`，实测 `SessionListProgress` 类型在 `:768`**；`:812` 的 `listSessionsFromDir(dir, onProgress, …)` 正确。异步、可报进度），pi-java 的 `list` 是**同步全量** | `docs/39 §6.4`（包⑫ 顺带发现，**未修**） | 对应的是**交互式选择器**那条车道（TUI/RPC 的 `-r` 面），本轮**没对照过**。⚠️ 别和 A12 混：A12 修的是**范围**，不是「把越界扫描做快」——**明确不做**并发/异步化（§6.3-3）。<br>⚠️ **2026-09-20 重测：差距从 1 项扩到 6 项**（pi `3390bd936`）：`SessionListProgress` 现于 **`:795`**，签名变为 **`(loaded, total, partialSessions?: readonly SessionInfo[]) => void`**；新增 ① 进度回调 ② **渐进发布部分结果**（`publishPartial` `:874-876`，首条 / 每 `CURRENT_SESSION_LIST_PUBLISH_INTERVAL = 10` / 收尾各发一次，带排序快照）③ **`AbortSignal`**（`:795/816/838/855`，`signal?.throwIfAborted()`）④ **并发扫描**（`buildSessionInfosWithConcurrency` `:835`）⑤ `listAll` 带 signal（`:1779`）⑥ 选择器接线（`main.ts:416-418`）。⇒ pi-java 的同步全量 `list` **多缺两个维度**（取消 + 渐进），不只是「无 onProgress」 |
| B56 | ⚠️ **`Usage` 的四个分量在生产上恒为 0** —— `cacheRead` / `cacheWrite` / `cacheWrite1h` / `reasoning` **全仓零生产者** | `docs/40 §2.1`（2026-09-20 实测，**本台账此前未登记**） | ✅ **包H1 已闭环（2026-09-22，`docs/42 §10`）**：`emitUsage(Usage)` 加宽（步 2）＋ 五车道四分量生产者（步 3–5，裁决 D 的 Google 无钳位照抄）＋ `UsageRecord` 全量下游接线（步 6，A3）。残留两条登记在 `docs/42 §10.4`（目录 cache 价数据＝裁决 B 的数据面；`cacheWrite1h` 真 key 端到端）。原描述：五条车道的 `emitUsage` 全部只传两个数（`AnthropicMessagesApi:286`、`GoogleGenerativeAiApi:141`、`MistralConversationsApi:251`、`OpenAICompletionsApi:182`/`:254`、`ResponsesStreamProcessor:247`）⇒ `Usage.of(a,b)` 把 `cacheRead`/`cacheWrite` **钉死 0**（`Usage.java:53-54`）。**而下游已在读**：`ContextOverflow.java:110`/`:120` 算 `input + cacheRead`、`ContextUsageEstimator.java:78` 算 `input + output + cacheRead + cacheWrite`、`SessionState.java:329-330` 累加两者。**后果**：开了 prompt caching 的请求（Anthropic 的 `cache_read` 常远大于 `input`）**上下文占用被低估 ⇒ 压缩触发晚于 pi**。落盘面已备好（`EntryJsonCodec.java:118-126`）只是没人填。**与 B24/B57 同属 usage 层，建议合并一个包** |
| B57 | ⚠️ **`calculateCost` 整段无对应物** ⇒ `Usage.Cost` 恒零 ⇒ **成本永远显示 0** | `docs/40 §2.1`（**未登记**） | ✅ **包H1 已闭环（2026-09-22，`docs/42 §10`）**：`CostCalculator.calculateCost` P16 逐条移植（步 1，四件「不照抄就会做错」的事全钉）＋ `PricingInfo` 五组件（裁决 A）＋ models.json `cost` 扩键/半价→UNKNOWN（步 6，裁决 F）。原描述：pi 有 `calculateCost`（`models.ts:891-911`，含 1h 缓存按 2× 输入价、`tiers` 阶梯价）；pi-java 全仓无对应物 ⇒ `SessionState.java:332` 与 `SessionRunner.java:78-79` 累加的 `cost().total()` **恒为 0**。`PricingInfo` 只有 input/output 两个分量，**不足以支撑**（pi 是四种）。与 B56 同域、**建议同包** |
...
> ### ⚠️ 2026-09-20 重测（pi `71dca871b` → `3390bd936`，111 提交）新登记 B67–B79
>
> 全部由 `docs/40 §1.3` 的 6 路重测撞出，**本台账此前无这些条目**。
> 三条最重的：**B67**（取代被删的 `addedToolNames`）、**B76/B77**（**判定由「对齐」翻为「缺失」**——
> pi 本轮自己前进了，pi-java 没跟）、**B78**（**判定更正**，与漂移无关，是原判没核算法）。
> 另有三块**整块零对应**：**B71 `pico3`（7,994）· B72 `durable`（757）· B73 `micro`（1,524）**
> ＝ 10,275 行，且⚠️ **三者在 pi 自己那边也没接线**（`durable` 无人 import；`pico3` 只被 `micro` 用；
> `micro` 无人 import）⇒ **一条自闭合的链，从 CLI 不可达**。

| B67 | ⚠️ **`SystemMessage` transcript 系统消息模型 ＋ 工具状态增量** —— **取代被删除的 `addedToolNames`** | `docs/40 §1.3`、`docs/map/02`（**未登记**） | pi 把「系统提示 ＋ 工具声明」从 **context 搬进消息流**：`SystemMessage`（`types.ts:491-507`，`Message` 联合纳入 `:553`）· `normalizeContext`/`TranscriptContext`（`utils/transcript.ts:27-30`、`types.ts:617-634`）· 工具状态增量（`toolsAdded`/`toolsRemoved` ＋ `getCurrentTools`/`resolveTranscriptTools`/`getToolStateChanges`/`declarationsEqual`/`hasToolRedefinitions`，`utils/transcript.ts:46-53,154-215`）· Anthropic 线格 `tool_addition`/`tool_removal`（`anthropic-messages.ts:1256-1268`）· 提示词分段 `sections` ＋ 渲染（`types.ts:496-501`、`utils/text.ts:15-40`）。**支持中途增改工具与系统提示** ⇒ pi-java 无对应物，**缺失**（权重 3）。⚠️ **C11/B4 的作废正是因为它** |
| B68 | **`declareToolChanges()` 工具装配变更声明** | `docs/map/03`（**未登记**） | `agent-loop.ts:281-331`，配套 pi-ai 的 `SystemMessage.toolsAdded/toolsRemoved` ⇒ **缺失**（权重 2） |
| B69 | **`prepareNextTurn` 返回 `messages`** | `docs/map/03`（**未登记**） | pi `AgentLoopTurnUpdate` 新增 `messages?`（`types.ts:146-147`、`agent-loop.ts:184-186,209-214`）；pi-java 的 `PiLoop.NextTurnUpdate(model, thinking, context)` 无此通道 ⇒ **缺失**（权重 2） |
| B70 | **`Agent.reset()` 保留基线 system 消息** | `docs/map/03`（**未登记**） | `agent.ts:344-355` vs `RunLifecycle.reset:217-235` ⇒ **存疑** |
| B71 | ⚠️ **`pico3` 子系统整块零对应（7,994 行）** | `docs/map/06`、`docs/map/03`（**未登记**） | 硬化的会话内核：Harness 812 · Session 1,497 · Scheduler 486 · 9 种 kinds 2,275 · System 376 · ViewManager 477 · Jsonl+Memory 617 · Membrane 123 · Bounded 100 · bashTool 29 · Chord 集成 178 · types 1,038。导出路径 `experimental/pico3`。⚠️ **pi 侧也只被 `micro` 用，而 `micro` 全仓无人 import** ⇒ **做不做是裁决项** |
| B72 | ⚠️ **`durable` 包整块零对应（757 行，pi 新包）** | `docs/map/06`、`docs/map/03`（**未登记**） | 会话/任务/文档持久运行时：四表模型（Conversation/Entry/Task/Input）· 三份文档 · `ConversationRecord.parent`/`owner` · **`ContextEdit`（omit/replace 的不可变覆写）** · `EntryRecord`（模型面与应用面分离）· `Input` 生命周期 ＋ `requestId` 去重 · `MemoryStorage`。公开 API 只有 `MemoryStorage` ＋ `ROOT_CONVERSATION_ID`。⚠️ **pi 全仓无人 import**（`pi-durable` 只被自己的 `package.json` 提到）⇒ **做不做是裁决项** |
| B73 | ⚠️ **`micro` 子系统整块零对应（1,524 行）** | `docs/map/05`、`docs/map/06`（**未登记**） | 单进程 Pico3 编码代理：会话目录布局 ＋ `--continue` · **`proper-lockfile` 文件系统锁**（`sessions.ts:39-44`，`retries:0` 直接抛 `Micro session is already open`）· Pico3 `JsonlStorage` 落盘。⚠️ **pi 全仓无人 import** ⇒ **做不做是裁决项** |
| B74 | **扩展 `on()` 不返回退订函数** —— 影响全部 **37** 条订阅签名 | `docs/map/04`（**未登记**） | pi 的 `ExtensionAPI.on()` 全部由 `): void` 改为 **`): () => void`**（`types.ts:1266-1326`），实现把 handler 从 `extension.handlers` 摘除（`extensions/loader.ts:256-270`）。pi-java 是装配期一次性注册、**无退订面** ⇒ **缺失**（权重 2） |
| B75 | **提示缓存预热（cache warming）整块缺失** | `docs/map/02`、`docs/map/04`（**未登记**） | pi `c596d09d9`（#9668）：`core/cache-warmer.ts` **453 行** ＋ `cacheWarming` settings 键（`settings-manager.ts:157`，`off\|streaming\|idle`，默认 `streaming`）＋ 扩展事件 `cache_warming_decision`（**pi 扩展事件 36 → 37**）＋ `Model.promptCache` 元数据（`types.ts:115,974`）＋ TUI 缓存预热用量行。⚠️ **与 B56 同域** —— pi 在 cache 这条线上加速、pi-java 原地 ⇒ **建议同包** |
| B76 | ⚠️ **`ContextOverflow` 溢出判定落后 pi 两个行为** | `docs/40 §1.3`、`docs/map/02`（**未登记**） | pi 本轮改了：① z.ai 模式 `/prompt is too long/i` 放宽为 `/prompt (?:is )?too long/i`（`utils/overflow.ts:38`）；② Cerebras bodyless 模式**移出** `OVERFLOW_PATTERNS`、改按 provider 门控（`:62` ＋ `:143-145`）。pi-java 停在旧语义（`ContextOverflow.java:39` 仍是 `prompt is too long`；`:63` Cerebras 模式**未门控**）⇒ ①「Prompt too long」**匹配不上** ②**任意 provider 都命中 Cerebras 模式**。⚠️ **判定由「对齐」翻为「缺失」** |
| B77 | ⚠️ **`RetryableError` 分类落后 pi 两个模式** | `docs/40 §1.3`、`docs/map/02`（**未登记**） | pi 新增 `"currently experiencing high demand"`（`utils/retry.ts:29`）、`"520"`（`:37`）；pi-java `RetryableError.java:41-112` 无这两个。⚠️ **判定由「对齐」翻为「缺失」** |
| B78 | ⚠️ **`FuzzyMatcher` 语义与 pi 不同**（模糊匹配） | `docs/40 §1.3`、`docs/map/05`（**未登记**） | pi 是**子序列**匹配 ＋ 按空白/斜杠**分词**（`fuzzy.ts:12,100`）；pi-java 是**子串**匹配 ＋ **无分词**（`FuzzyMatcher.java:31`：`startsWith`→0、`indexOf`→下标、否则不匹配）。例：query `abc` vs `a-b-c` —— **pi 命中、pi-java 不命中**。挂载点也错位：pi 的 `fuzzyFilter` 服务 autocomplete/session-selector/tree-selector，pi-java 的只服务 `SelectList.java:142`；而 pi 的 `select-list.setFilter`（`select-list.ts:61`）用纯 `startsWith` ⇒ **三处错位**。⚠️ **判定由「对齐」翻为「存疑」，且与 pi 的 111 提交无关** —— 是重测时把两侧算法读透才发现的**原判未核算法** |
| B79 | **`/bug` 故障上报 ＋ 崩溃记录**（pi 的 slash 命令 **23 → 24**） | `docs/map/04`、`docs/map/05`（**未登记**） | pi：`/bug` 流程（同意提示 → 可选摘要 → 上传或导出 zip：`bug-report.ts:294` ＋ `core/bug-report.ts` 375 ＋ `bug-report-upload.ts` 37 ＝ **706 行**）＋ 崩溃记录与下次启动提示（`core/crash-log.ts` 92 行，`takeUnnotifiedCrash`/`recordCrash`，`/bug` 提示每会话最多一次）。pi-java 无 ⇒ **缺失**（权重 1）。<br>同批 pi 新增的 TUI 小项（权重 1，一并记此行）：`R24` 缓存预热用量行 · `R25` thinking 块被丢弃提示（`Anthropic dropped N thinking blocks`）· `K37` 消息卡鼠标点击展开/折叠 |
| B80 | **Mistral 车道的数组形态 `delta.content` 根本没有解析路径** | 包 A0 步5 实读（`docs/43 §6-9`） | pi `mistral-conversations.ts:621-680` 对 `delta.content` 有**三条分支**（字符串项 ／ `type:"thinking"` 项 ／ `type:"text"` 项）；pi-java `MistralConversationsApi:204` 只按 `(String) delta.get("content")` 读 ⇒ ① `thinking` 增量**永不产生 `ThinkingDelta`**（推理内容整段丢失）；② 送数组形态时**抛 `ClassCastException`**（不是丢内容，是炸在事件处理里）。⚠️ 顺带：pi 在这三处都做 `sanitizeSurrogates`（A0 只落得下字符串那一处）。修法：按 pi 逐分支移植（含净化），归「Mistral 车道补齐」包 |
| B81 | **pi OAuth 分支的两个面未移植**（系统提示前缀 ＋ 工具名映射） | 包 A0 步8 实读（`docs/43 §6-10`） | pi `anthropic-messages.ts:1078-1090`：`isOAuthToken` 为真时 `params.system` **前置**一块 `"You are Claude Code, Anthropic's official CLI for Claude."`；`:1255-1262` 工具名与 `:1352` tool_use 名经 `toClaudeCodeName(...)`。pi-java 两处都没有（A0 落的是**凭证形态＋身份头**，D7 明确划在这两面之外）。后果：OAuth 形态下请求形状与 pi 不同（服务端策略可能因此不同）。另两枚浏览器头（`accept`／`anthropic-dangerous-direct-browser-access`）**有意不移植**（java 无浏览器场景） |
| B82 | **OAuth 登录的结果不在凭证链的读取路径上** | 包 A0 步7 实读（`docs/43 §6`；`AuthCommand:69` vs `Credentials:24-41`） | `AuthCommand` 的 OAuth 流程把凭证写进 `~/.pi-java/auth-oauth.json`（`OAuthCredentialStore:28-29`，**只有 `AuthCommand` 读写它**）；而请求路径的凭证链读的是 `~/.pi-java/auth.json`（`FileCredentialStore`）＋ 环境变量 ⇒ **登录成功对请求没有任何影响**（今天：跑完 `auth login anthropic`，车道仍报无 key）。pi 侧 OAuth 凭证进的就是请求路径（`oauth.ts` 解析后交给 provider）。修法：让 `Credentials` 在 OAuth 文件里也找（并把形态定成 `AuthKind.OAUTH`）；A0 已把形态载体铺好，只差接线 |
| B84 | **Google 车道的工具结果路径整条缺失** —— 工具结果被当成一条 **model 轮**的纯文本发出去 | 包 H2 步6 实测（`docs/44 §10-1`） | **实测证据**（临时夹具把真请求体打出来，同一份 user → assistant 调 `ls` → toolResult `"file-a.txt"`）：pi-java 发 `{"parts":[{"text":"file-a.txt"}],"role":"model"}`；pi 发 `{"role":"user","parts":[{"functionResponse":{"name":"ls","response":{"output":"file-a.txt"}}}]}`（`google-shared.ts:280-340`）。即 `functionResponse` **根本不存在** ⇒ **Gemini 的多轮工具调用今天就是坏的**（模型看不到工具结果与调用的对应关系）。三处连带：① `toGoogleParts` 里那个 `ContentBlock.ToolResultContent` 分支**实际不可达**（`toGoogleContents` 对 `ToolResultMessage` 走 `msg instanceof UserMessage ? "user" : "model"` ⇒ 落到 model 轮、且只遍历 `content()` 的原始块）；② 真要走那个分支，它把 `toolUseId` 传给了 `Part.fromFunctionResponse(name, …)` 的 **name** 形参（`javap` 实测第一个形参就是 name）⇒ 发出去的 name 是**工具 id 而不是工具名**；③ pi 的 `response` 键是 `output`/`error`，java 写的是 `content`。**修法＝整块移植**：`functionResponse` 全形（name/output/error/id）＋ 能力门 ＋ `supportsMultimodalFunctionResponse`（Gemini 3+ 内嵌图片 / <3 另起 user 回合）＋ 连续 functionResponse 合并进同一 user 回合。约 100 行 ＋ 一组夹具，**须自己一份设计**（用户 2026-09-22 裁决拆出 H2）。<br>✅ **已实施（包B84，`docs/45`，2026-09-23，六提交 `efedec4`(设计) `a22ccca`(步1) `f77dfb4`(步2) `2b46773`(步3) `b1bf735`(步4) `4890057`(步5) ＋ 台账）**：`toGoogleContents` 按 pi 的**三分支**重写（user／assistant／toolResult）＋ 工具结果整块照抄（name 取工具名／`{output,error}` 二选一／`responseValue` 三选一／合并进同一 user 回合／gemini<3 独立 `"Tool result image:"` 回合／`id` 门）。**四处超出原记范围、均已做**：① **升 `google-genai` 1.15.0 ⇒ 1.72.0** —— 1.15.0 的 `FunctionResponse` **没有 `parts` 字段**（`javap`），且 `Part.fromJson` **静默丢弃**未知键 ⇒ pi 的 gemini 3+ 内嵌图片**写不出来**（实测 1.72.0 有 `FunctionResponsePart`，且升级后 671/671 绿、十类 `javap` 只增不减）；② `functionCall.id` 门（assistant 侧，同一个谓词的同一道门）；③ **空白助手文本块跳过**（pi `:240`，真链路可达 —— 两侧流式侧都会造出 `text:""`）；④ 拆出 `GoogleMessageConverter`（车道 407⇒325，转换器 346，两者 ≤500）＋ `ToolResultContent` 块级分支**置空**（解码专用死类型，生产零构造点）。**新增夹具 3 个类**（路由／助手重放／谓词 ＋ 共用的 `GoogleWireBody`），骨架**镜像 pi 自己的** `google-shared-image-tool-result-routing.test.ts` ⇒ 跨实现 oracle。ai 643⇒**671** |
| B85 | **`read.ts` 侧的两条未移植**（图片自动缩放 ＋ 非视觉模型注记） | 包 H2 实读（`docs/44 §7 待裁决④`，用户裁决「只登记」） | ① `processImage(buffer, mimeType, {autoResizeImages})`（`coding-agent/src/utils/image-process.ts`，`read.ts:112`）把图片自动缩到 **2000×2000 上限**；pi-java 的 `ReadTool` 把原图 **base64 原样**送出 ⇒ 大图会撑大请求体与 token（无上限、无降级）。② `getNonVisionImageNote`（`read.ts:57-62`）在模型不支持图片时给工具文本追加 `"[Current model does not support images. The image will be omitted from this request.]"`；pi-java 零对应物 ⇒ 非视觉模型只看到 `"Read image file [image/png]"`（H2 的闸会把图片换成占位文本，但**工具文本里没有那句解释**）。两条都在 `agent-core`／`coding-agent`，不属 `ai` 模块 |
| B86 | **包 H2 顺带发现的形状偏差（三处）＋ 两处不可观察的能力门** | 包 H2 实读（`docs/44 §6/§10-3`） | **形状偏差**（均无用户可见症状，本包不动）：① **user 消息的串形态** —— pi 的串分支（`typeof msg.content === "string"`）在 pi-java **结构上不可达**（`UserMessage.content` 恒为 `List`），故 pi-java 在无图片时发**字符串**、pi 会发**块数组**（`openai-completions.ts:1259-1272` ／ `mistral-conversations.ts:798-806` ／ `responses:236-250`）；H2 只在**有图片**时改用数组分支；② **responses 的 `toUserItem` 过滤空文本块**（`ResponsesMessageConverter:160`）而 pi 不过滤（`openai-responses-shared.ts:238-250`）；③ **`ModelCompat` 缺 `requiresToolResultName` ／ `requiresAssistantAfterToolResult`**（pi `openai-completions.ts:1642-1643` 默认 false）—— java 的 `ModelCompat` 是**逐旗标按消费者移植**的（`ModelCompat.java:6-11` 明文），而这两旗标无 `models.json` 解析面 ⇒ 加进来恒为 false ＝ 死码。<br>⚠️ **两处不可观察的能力门**（**照抄保留，别当死码删**）：`openai-completions.ts:1424` 与 `openai-responses-shared.ts:91` 各有一道独立于共享闸的图片门；**实测去掉它零红** —— 共享闸已按**同一个 model** 把非视觉模型的图片换成文本块 ⇒ 车道永远收不到图片。pi 两侧都保留，故照抄；「零红」不是夹具没牙，是这一行**没有出参**（`docs/44 §10-3`）。<br>⚠️ **2026-09-23 包B84 补第 3 例**：Google 车道侧的能力门（`GoogleMessageConverter.addToolResult` 的 `model.supportsImageInput()`，pi `google-shared.ts:288`）同形 —— **实测把整个判据换成 `true` 零红**（`docs/45 §10-3`）。⇒ 本条的「两处」应读作「**三处**」（completions／responses／google）。<br>⚠️ **2026-09-23 包B84 追加两条登记**：① **`google-genai` 的 native-image 未验证** —— 1.72.0 新引入 `com.google.genai.gaos.*` 等新包的反射配置未核（native 路线此前已判「不现实」，见 memory）；② **空白判据的 NBSP 缝** —— pi 的 `trim()` 剥 U+00A0(NBSP)／U+FEFF，Java 的 `isBlank()` 不认 ⇒ **纯 NBSP** 的助手文本块在 pi 被跳过、在 java 上线（可达性窄：需 provider 发纯 NBSP 块；`isBlank()` 已是三种 Java 写法里最接近 pi 的，`docs/45 §10-4`） |
...
| C11 | `addedToolNames` 的 **provider 层消费者**（原 B4） | `docs/31:3749`（§8.32.4） | **结案为不做**，且**机制归属原判是错的**：`addedToolNames` 来自 pi 的**扩展系统**（`extensions/wrapper.ts:17-37` → `deferred-tools.ts:8-39`），不是 MCP；消费侧的闸是 `openai-completions.ts:838-840` 的 `compat.deferredToolsMode === "kimi"` | ① 扩展系统落地且真产出 `addedToolNames`；② 出现 `deferredToolsMode` 为真的 provider；③ 有用户报告 deferred tools 不生效 |<br>⚠️ **2026-09-20 作废（pi 已删除）** —— `grep -rn addedToolNames packages/` 在 pi 新 HEAD **全仓零命中**：该字段连同 `compat.deferredToolsMode`、`AnthropicMessagesCompat.supportsToolReferences`、整个 `utils/deferred-tools.ts` **文件**都被删了，机制被「系统消息工具增量」（`SystemMessage.toolsAdded/toolsRemoved`，`types.ts:491-507`）取代。**按裁决 R1（pi 删掉的，pi-java 也删）本行作废，且触发条件①永不成立。** 取代它的新缺口见 **B67** |
...
| E6 | 拆分超限文件（`SqliteSessionStorage`、`AgentHarness`） | `docs/09b:117` | ⚠️ **2026-09-20 更正：`SqliteSessionStorage` 部分已解决** —— `docs/09b §5` 已压到 498 行、**实测现为 463 行**（已拆出 `storage/` 子包 10 文件）⇒ **本行只剩 `AgentHarness`（＝ E4）**，建议合并进 E4。<br>⚠️ **2026-09-22 包 H2 追加**：`pi-java-ai` 的 `AnthropicMessagesApi`（**696**）与 `OpenAICompletionsApi`（**729**）也曾超限（本包各加 ~70 行触发）⇒ 已各抽出一个**纯函数转换器**（`AnthropicMessageConverter` 227 ／ `OpenAICompletionsMessageConverter` 391），现为 **494 ／ 366**，`protocol` 包全部 ≤ 500 行（最大 `MistralConversationsApi` 500） |
