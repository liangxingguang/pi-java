...
| B46 | ⚠️ **pi-java 的内置工具没有一条调 `onUpdate`** ⇒ `tool_execution_update` **在生产上永不发射**（只有测试桩发） | `docs/34 §4-F` | pi 侧 7 个工具全都有（`bash.ts`/`edit.ts`/`find.ts`/`grep.ts`/`ls.ts`/`read.ts`/`write.ts`）；pi-java 侧生产 grep `onUpdate.onUpdate` **零命中**（唯二命中是 `PiLoop:71` 的 record 定义与 `PiToolRunner:158` 的 emit，都不是工具侧调用）。⇒ 包⑦ 补好的 `tool_execution_update` 通路**只有测试桩能行使**。**登记不修**（包⑦ 范围外）：要给 7 个工具各自决定「什么是部分结果」（pi 的 bash 发累计快照 `bash.ts:260-272`、还会先发一条空 update），是**独立一包** |
