...
|---|---:|---:|---:|---:|---:|---|
| `pi-java-agent-core` | 204 | 131.5 | **64.5%** | 37.0% | **83.3%** | P0 |
| `pi-java-ai` | 200 | 119.5 | **59.75%** | 43.5% | 75.9% | P0 |
| `pi-java-telemetry` | 35 | 17.5 | **50.0%** | 41.2% | 64.3% | P2 |
| `pi-java-tui` | 229 | 108.5 | **47.4%** | 30.1% | 67.3% | **P3** |
| `pi-java-coding-agent` | 446 | 200.0 | **44.8%** | 32.7% | **41.3%** | P0 |
| `pi-java-session-backend-sqlite`＋`evals` | 154 | 56.5 | **36.7%** | 35.5% | 34.1% | P1 |
| **合计** | **1268** | **633.5** | **49.96%** | 36.5% | **≈62%** | |
| ＊含 chord＋多进程（若裁决做） | 1334 | 633.5 | **47.5%** | — | — | 待裁 |
...
|---|---:|---|---|---|
| **`examples/` 可执行验收规格** | **15,912** | 0 | 102 个 `.ts` / 79 个扩展示例 —— 它同时是 pi 扩展 API 的**可执行定义** | **P1** |
| **`chord` RPC 基座** | **5,822**（+3,553 测试） | 0 | facet / 复制状态 / 服务 RPC / delta 词汇 | **待裁** |
| **pi 防漂移脚本** | **8,314**（40 文件） | 0（部分替代） | 其中 **8 个是真漂移门**；pi-java 用 Checkstyle/SpotBugs/enforcer ＋ **L5 差分**（pi 没有，比 pi 任何脚本都强） | P2 |
| **多进程子系统** | ~3,000 | 0 | coordinator / session-worker / Unix socket 三层拓扑 | **待裁** |

**待裁项对总分的影响**：裁掉 chord＋多进程把存储层从 **25.7% 抬到 36.7%（+11.0pp）**
（它们 66 点权重**全落在缺失**，分子一分不加）。
...
| 模块 | pi LOC | java LOC |
|---|---:|---:|
| `coding-agent` | 70,052 | 10,325 |
| `tui`（＋app 层） | 39,010 | 6,712 |
| `agent-core` | 25,305 | 16,386 |
| `ai` | 24,383 | 11,590 |
| `session-backends` | 1,973 | 2,875 |
| `evals` | 1,964 | 773 |
| `telemetry` | 935 | 787 |
| **合计** | **163,622** | **48,661** |
...
| 阶段 | 内容 | 为什么 |
|---|---|---|
| **A. 删（裁决 R1）** | 旧 schemas 协议层 ＋ `SessionHandle` ＋ `writer_leases` 机制（连带 A17） | 零连带；消掉 85 个虚假单元和 1 个查不到的 flake |
| **B. 台账重建** | 修 §7 的 19 条错 ＋ 补 §8 的 10 条漏 | 台账是唯一索引，它错则计划必错 |
| **C. P0 修行为** | 按 §2 的权重 3 清单：① usage 域两条（+5.50pp，76% 收益）② 图片三车道 ③ `openai` 默认 wire ④ 上下文文件发现 ⑤ 扩展钩子桥接 ⑥ B15 打通 ⑦ `/compact` abort-first | 全在默认路径上 |
| **D. P1 补缺口** | ① JSONL v4 ＋ Entry v4（同根因）② SQLite schema（需先裁决范式）③ `entry_appended` 形状 | |
| **E. 待裁项** | chord 5,822 要不要做？多进程要不要做？SQLite 跟 pi 的 4-kind 模型还是保留 java 的？RPC 要不要做？ | 决定后续工作量级（chord＋多进程一项 = ±11.0pp） |
| **F. P3** | TUI（裁决 R3 排最后）。**但 §4 的两条死码先决定：接线还是删声明** | — |
...
- `docs/01`/`02`/`04`/`phase1-pi-code-mapping` 的过期未修。
