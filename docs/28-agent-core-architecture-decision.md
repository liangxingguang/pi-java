...
| — | 🔄 **进行中**。第一子句经核对**已由第 2 步的接线满足**（`LaneRecord` 11 个变体在新路径上都有发射点，对照表见 `docs/29 §9.1`）。补空白覆盖时发现 `docs/29 §9.2` 的既有缺陷，并确认**折叠模型的 pi 参照已不存在**（`docs/29 §9.3`）⇒ 裁决**退休**，实施蓝图见 **`docs/30`**。核对过程中另证实一个**独立 P0**：崩溃恢复的车道在新驱动下**不可驱动**（`PiLaneEngine.run` 拒绝非 IDLE 车道，而 `TryFinishRun` 只存在于旧步进链）—— `docs/30 §3`，由 `docs/30 §5` Step 1 单独修复 | |
