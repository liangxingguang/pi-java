...
> **⚠️ 勘误（2026-09-12，见 `docs/22-deferred-writes-and-entry-provenance-design.md` §1.3）**：
> 下表 **#2 与 #3 的「为什么不做」理由经代码核对后不成立或仅部分成立**，已在 docs/22 中实施：
...
> - **#1 / #4** 已在 docs/22 实施（#1 限定 harness/记录层，#4 删 `StepAttempt.stopReason` 改 entry 唯一真相）。
