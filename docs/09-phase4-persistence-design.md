...
- **可选字段省略（对齐 pi `?:` 字段）**：pi 的 `terminate?`/`details?`/`usage?`/`data?`/`systemPromptOverride?` 等可选字段在 JSON 中**缺省即省略**。Java 用**可空引用 + `@JsonInclude(NON_NULL)`** 表达：`terminate` 用 `Boolean`（存在即 `true`，省略即 null）、`details`/`usage`/`data` 用可空引用。`@JsonInclude(JsonInclude.Include.NON_NULL)` 配在 `Entry`/`LaneRecord` 接口上，保证与 pi codec 字节级一致（§3.4 抽样断言）。
- **`parentId` 例外（必须恒输出键）**：pi 的 `EntryBase.parentId` 是 `string | null`（非可选），`codec.ts` 的 `requireNullableId(value.parentId, "parentId")` 对**缺失键**直接抛 schema 错。因此根 entry 必须输出 `"parentId":null`，编码层在 `parentId` 为 null 时显式补键（不可依赖 NON_NULL 省略），§4.2 示例（`"parentId":null`）即此要求。基础字段（`id/seq/type/lane/timestamp`）永非空。
